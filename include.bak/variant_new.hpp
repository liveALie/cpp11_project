...
template <int index, typename... Types> struct At;
...
template <typename T, typename... Types> struct At<0, T, Types...> {
  using type = T;
};
...
public:
  template <int index> using IndexType = typename At<index, Types...>::type;

  Variant(void) : type_index_(typeid(void)) {}

  ~Variant() { destroy(type_index_, &data_); }
  ...
  template <class T, class = typename std::enable_if<Contains<
  ...
  bool Empty() const { return type_index_ == std::type_index(typeid(void)); }

  std::type_index type() const { return type_index_; }
  ...
  template <typename T> int indexOf() { return IndexOf<T, Types...>::value; }
...
private:
  ...
  template <typename T> void destroy0(const std::type_index &id, void *data) {
