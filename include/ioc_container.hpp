...
public:
  ...
  RegisterType(const std::string &key) {
    ...
    // std::cout << "register type:" << typeid(decltype(func)).name() <<
    // std::endl;  std::cout << "register type:" <<
    // typeid(std::function<T*(Args...)>).name() << std::endl;
    RegisterType(key, func);
  }
  //注册构造普通类型的对象的构造函数
...
private:
  ...
  void RegisterType(const std::string &key, Any creater) {
    // std::cout << "RegisterType type index name:" <<
    // creater.GetTypeIndexName() << std::endl;
