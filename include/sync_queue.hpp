...
public:
    SyncQueue(int maxSize)
        :max_size_(maxSize),need_stop_(false)
    {
    }
...
private:
    ...
    bool NotEmpty()const
    {
        ...
        if(empty){
            cout << "storage memory is empty,thread id:" << std::this_thread::get_id() << endl;
        }
        return !empty;
    }
    ...
    void Add(F&& x)
    {
        ...
        //反过来理解，wait not full，若not_full或停止了，往下执行，否则，unique_lock会锁住，等到没满为止。
