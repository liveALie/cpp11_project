...
public:
    ...
    typename std::enable_if<has_member_Before<T,Args...>::value
        && has_member_After<T,Args...>::value>::type Invoke(Args&&... args,T&& aspect)//这里用于自动推导，所以使用了&&
    {
...
{
    Aspect<F,Args...> asp(std::forward<F>(f));//先创建待切面的对象，参数为函数对象。
    asp.Invoke(std::forward<Args>(args)...,identify_t<AP>()...);//再调用所有的切面函数，这里根据切面类型，构造了ap对象。
}
