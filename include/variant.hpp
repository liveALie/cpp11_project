...
// 		// the i-th argument is equivalent to the i-th tuple element of
// a tuple
...
//��ȡ��������
template <size_t arg, size_t... rest> struct IntegerMax;
...
struct IntegerMax<arg1, arg2, rest...>
  ...
  /*static const size_t value = arg1 >= arg2 ? static_max<arg1,
  others...>::value : static_max<arg2, others...>::value;*/
};

template <typename... Args>
struct MaxAlign
    : std::integral_constant<
          int, IntegerMax<std::alignment_of<Args>::value...>::value> {};
/*
template<typename T, typename... Args>
struct MaxAlign : std::integral_constant<int,
(std::alignment_of<T>::value >MaxAlign<Args...>::value ?
std::alignment_of<T>::value : MaxAlign<Args...>::value) >
{};
...
//��ȡ��һ��T������λ��
// Forward
template <typename Type, typename... Types> struct GetLeftSize;
...
struct GetLeftSize<Type, Type, Types...>
    : std::integral_constant<int, sizeof...(Types)> {
...
struct Index
    : std::integral_constant<int, sizeof...(Types) -
                                      GetLeftSize<T, Types...>::value - 1> {};

//����������ȡ����λ�õ�����
// Forward declaration
template <int index, typename... Types> struct IndexType;
...
template <typename T, typename... Args> struct VariantHelper<T, Args...> {
  ...
  inline static void copy(std::type_index old_t, const void *old_v,
                          void *new_v) {
    if (old_t == std::type_index(typeid(T)))
...
public:
  ...
  template <class T, class = typename std::enable_if<
                         Contains<typename std::remove_reference<T>::type,
                                  Types...>::value>::type>
  ...
  bool Empty() const { return m_typeIndex == std::type_index(typeid(void)); }

  std::type_index Type() const { return m_typeIndex; }

  template <typename T> typename std::decay<T>::type &Get() {
  ...
  template <typename F> void Visit(F &&f) {
    typedef typename std::tuple_element<
        0, typename function_traits<F>::ArgTupleType>::type T;
  ...
  template <typename F, typename... Rest> void Visit(F &&f, Rest &&... rest) {
    // using T = typename function_trait<F>::(typename arg<0>)::type;
